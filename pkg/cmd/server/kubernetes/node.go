...
func (c *NodeConfig) RunProxy() {
	...
	case "iptables":
		...
		proxierIptables, err := iptables.NewProxier(iptInterface, exec, c.ProxyConfig.SyncPeriod, c.ProxyConfig.MasqueradeAll)
		if err != nil {
	...
	case "userspace":
		...
		proxierUserspace, err := userspace.NewProxier(loadBalancer, c.ProxyConfig.BindAddress, iptInterface, c.ProxyConfig.PortRange, c.ProxyConfig.SyncPeriod)
		if err != nil {
			// This should be fatal, but that would break the integration tests
			glog.Warningf("WARNING: Could not initialize Kubernetes Proxy. You must run this process as root to use the service proxy: %v", err)
			return
