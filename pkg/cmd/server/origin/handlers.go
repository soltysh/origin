...
func forbidden(reason string, attributes authorizer.AuthorizationAttributes, w http.ResponseWriter, req *http.Request) {
	kind := ""
	name := ""
	apiVersion := klatest.GroupOrDie("").GroupVersion
	...
	if attributes != nil {
		apiVersion = unversioned.GroupVersion{Version: attributes.GetAPIVersion()}
	...
	codec := klatest.GroupOrDie("").Codec
	if requestedCodec, err := klatest.GroupOrDie("").InterfacesFor(apiVersion); err == nil {
