...
func (c *MasterConfig) Run(controllerPlug plug.Plug, stopCh <-chan struct{}) error {
	...
	aggregatedAPIServer, err := c.withAggregator(delegateAPIServer, apiserver.RecommendedConfig{Config: *c.kubeAPIServerConfig.GenericConfig}, apiExtensionsInformers)
	if err != nil {
		return err
	}
