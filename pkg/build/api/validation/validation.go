...
func ValidateBuild(build *buildapi.Build) field.ErrorList {
	allErrs := validation.ValidateObjectMeta(&build.ObjectMeta, true, validation.NameIsDNSSubdomain, field.NewPath("metadata"))
...
func ValidateBuildUpdate(build *buildapi.Build, older *buildapi.Build) field.ErrorList {
	allErrs := validation.ValidateObjectMetaUpdate(&build.ObjectMeta, &older.ObjectMeta, field.NewPath("metadata"))
...
func ValidateBuildConfig(config *buildapi.BuildConfig) field.ErrorList {
	...
	triggersPath := field.NewPath("triggers")
	for i, trg := range config.Spec.Triggers {
		...
		if exists {
			allErrs = append(allErrs, field.Invalid(field.NewPath("triggers"), config.Spec.Triggers, "multiple ImageChange triggers refer to the same image stream tag"))
		}
		fromRefs[fromKey] = struct{}{}
	}

	allErrs = append(allErrs, validateBuildSpec(&config.Spec.BuildSpec, field.NewPath("spec"))...)
	...
	if strategy.DockerStrategy != nil && strategy.DockerStrategy.From == nil {
		for _, trigger := range config.Spec.Triggers {
			if trigger.Type == buildapi.ImageChangeBuildTriggerType && (trigger.ImageChange == nil || trigger.ImageChange.From == nil) {
				allErrs = append(allErrs, field.Required(field.NewPath("imageChange", "from")))
			}
		}
	}
...
func ValidateBuildConfigUpdate(config *buildapi.BuildConfig, older *buildapi.BuildConfig) field.ErrorList {
	allErrs := validation.ValidateObjectMetaUpdate(&config.ObjectMeta, &older.ObjectMeta, field.NewPath("metadata"))
...
func validateBuildSpec(spec *buildapi.BuildSpec, fldPath *field.Path) field.ErrorList {
	...
	allErrs = append(allErrs, validateSource(&spec.Source, s.CustomStrategy != nil, fldPath.Child("source"))...)
...
func validateSource(input *buildapi.BuildSource, isCustomStrategy bool, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	...
	if input.Image != nil {
		allErrs = append(allErrs, validateImageSource(input.Image, fldPath.Child("image"))...)
	}
