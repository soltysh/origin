...
func validateImage(image *api.Image, fldPath *field.Path) field.ErrorList {
	...
	result := validation.ValidateObjectMeta(&image.ObjectMeta, false, oapi.MinimalNameRequirements, metadataPath)

	var dockerImageReferencePath *field.Path
	if fldPath != nil {
		dockerImageReferencePath = fldPath.Child("dockerImageReference")
	} else {
		dockerImageReferencePath = field.NewPath("dockerImageReference")
	}
	if len(image.DockerImageReference) == 0 {
		result = append(result, field.Required(dockerImageReferencePath))
	} else {
		if _, err := api.ParseDockerImageReference(image.DockerImageReference); err != nil {
			result = append(result, field.Invalid(dockerImageReferencePath, image.DockerImageReference, err.Error()))
		}
	}
