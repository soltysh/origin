...
func (c *policies) List(opts kapi.ListOptions) (result *authorizationapi.PolicyList, err error) {
	...
	err = c.r.Get().Namespace(c.ns).Resource("policies").VersionedParams(&opts, authorizationapi.Scheme).Do().Into(result)
	return
}
...
func (c *policies) Watch(opts kapi.ListOptions) (watch.Interface, error) {
	return c.r.Get().Prefix("watch").Namespace(c.ns).Resource("policies").VersionedParams(&opts, authorizationapi.Scheme).Watch()
}
