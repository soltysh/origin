...
func (c *clusterPolicies) List(opts kapi.ListOptions) (result *authorizationapi.ClusterPolicyList, err error) {
	...
	err = c.r.Get().Resource("clusterPolicies").VersionedParams(&opts, authorizationapi.Scheme).Do().Into(result)
	return
}
...
func (c *clusterPolicies) Watch(opts kapi.ListOptions) (watch.Interface, error) {
	return c.r.Get().Prefix("watch").Resource("clusterPolicies").VersionedParams(&opts, authorizationapi.Scheme).Watch()
}
