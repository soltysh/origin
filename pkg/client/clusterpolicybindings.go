...
func (c *clusterPolicyBindings) List(opts kapi.ListOptions) (result *authorizationapi.ClusterPolicyBindingList, err error) {
	...
	err = c.r.Get().Resource("clusterPolicyBindings").VersionedParams(&opts, authorizationapi.Scheme).Do().Into(result)
	return
}
...
func (c *clusterPolicyBindings) Watch(opts kapi.ListOptions) (watch.Interface, error) {
	return c.r.Get().Prefix("watch").Resource("clusterPolicyBindings").VersionedParams(&opts, authorizationapi.Scheme).Watch()
}
