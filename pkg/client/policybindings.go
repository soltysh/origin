...
func (c *policyBindings) List(opts kapi.ListOptions) (result *authorizationapi.PolicyBindingList, err error) {
	...
	err = c.r.Get().Namespace(c.ns).Resource("policyBindings").VersionedParams(&opts, authorizationapi.Scheme).Do().Into(result)
	return
}
...
func (c *policyBindings) Watch(opts kapi.ListOptions) (watch.Interface, error) {
	return c.r.Get().Prefix("watch").Namespace(c.ns).Resource("policyBindings").VersionedParams(&opts, authorizationapi.Scheme).Watch()
}
