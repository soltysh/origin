...
func (m *VirtualStorage) updateRoleBinding(ctx kapi.Context, originalObj runtime.Object, allowEscalation bool) (*authorizationapi.RoleBinding, bool, error) {
	var updatedRoleBinding *authorizationapi.RoleBinding
	var roleBindingConflicted = false

	var name = ""
	if r, ok := originalObj.(*authorizationapi.RoleBinding); !ok {
		return nil, false, kapierrors.NewBadRequest(fmt.Sprintf("obj is not a role binding: %#v", originalObj))
	} else {
		name = r.Name
	}
