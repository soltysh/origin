...
func TestRepositoryBlobStat(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		if !reflect.DeepEqual(err, tc.expectedError) {
			t.Errorf("[%s] got unexpected error: %s", tc.name, util.ObjectGoPrintDiff(err, tc.expectedError))
			continue
		}
		if tc.expectedError == nil && !reflect.DeepEqual(desc, tc.expectedDescriptor) {
			t.Errorf("[%s] got unexpected descriptor: %s", tc.name, util.ObjectGoPrintDiff(desc, tc.expectedDescriptor))
		}
	...
	for imageReference := range repoImages {
		...
		for i := 0; i < imageCount; i++ {
			...
			for _, l := range img.DockerImageLayers {
				t.Logf("  %s of size %d", l.Name, l.Size)
			}
