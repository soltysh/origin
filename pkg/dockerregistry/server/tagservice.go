...
func (t tagService) Tag(ctx context.Context, tag string, dgst distribution.Descriptor) error {
	...
	image, err := t.repo.registryClient.Images().Get(dgst.Digest.String())
	if err != nil {
	...
	return t.repo.registryClient.ImageStreamMappings(imageStream.Namespace).Create(&ism)
}
...
func (t tagService) Untag(ctx context.Context, tag string) error {
	...
	return t.repo.registryClient.ImageStreamTags(imageStream.Namespace).Delete(imageStream.Name, tag)
}
