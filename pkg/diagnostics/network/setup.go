...
func (d *NetworkDiagnostic) TestSetup() error {
	...
	for _, name := range nsList {
		...
		ns := &kapi.Namespace{ObjectMeta: kapi.ObjectMeta{Name: name, Annotations: map[string]string{"openshift.io/node-selector": ""}}}
	...
	for _, node := range d.nodes {
		for _, nsName := range nsList {
			...
			for i := 0; i < 2; i++ {
				...
				if _, err := d.KubeClient.Pods(nsName).Create(pod); err != nil {
			...
			if _, err := d.KubeClient.Services(nsName).Create(service); err != nil {
