...
func (eg *egressIPInfo) addNode(node *nodeEgress) {
	if len(eg.nodes) != 0 {
		glog.Errorf("Multiple nodes claiming EgressIP %q (nodes %q, %q)", eg.ip, node.nodeIP, eg.nodes[0].nodeIP)
	}
...
func (eg *egressIPInfo) addNamespace(ns *namespaceEgress) {
	if len(eg.namespaces) != 0 {
		glog.Errorf("Multiple namespaces claiming EgressIP %q (NetIDs %d, %d)", eg.ip, ns.vnid, eg.namespaces[0].vnid)
	}
	...
	if egressIPActive && eg.assignedNodeIP != eg.nodes[0].nodeIP {
		...
		if eg.assignedNodeIP == eip.localIP {
			if err := eip.assignEgressIP(eg.ip, eg.assignedIPTablesMark); err != nil {
				glog.Errorf("Error assigning Egress IP %q: %v", eg.ip, err)
				eg.assignedNodeIP = ""
			}
		}
		...
		if eg.assignedNodeIP == eip.localIP {
			if err := eip.releaseEgressIP(eg.ip, eg.assignedIPTablesMark); err != nil {
				glog.Errorf("Error releasing Egress IP %q: %v", eg.ip, err)
			}
		}
		eg.assignedNodeIP = ""
