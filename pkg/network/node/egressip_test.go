...
func assertOVSChanges(eip *egressIPWatcher, flows *[]string, changes ...egressOVSChange) error {
	oldFlows := *flows
	newFlows, err := eip.oc.ovs.DumpFlows()
	if err != nil {
	...
	for _, change := range changes {
		...
		for _, flow := range *flows {
			if strings.Contains(flow, "table=100") && strings.Contains(flow, vnidStr) {
		...
		case Dropped:
			flowChanges = append(flowChanges,
				flowChange{
					kind:  flowAdded,
					match: []string{"table=100", vnidStr, "drop"},
				},
			)
		case Local:
			flowChanges = append(flowChanges,
				flowChange{
					kind:  flowAdded,
					match: []string{"table=100", vnidStr, fmt.Sprintf("%s->pkt_mark", getMarkForVNID(change.vnid, eip.masqueradeBit)), "goto_table:101"},
				},
			)
		case Remote:
			flowChanges = append(flowChanges,
				flowChange{
					kind:  flowAdded,
					match: []string{"table=100", vnidStr, fmt.Sprintf("%s->tun_dst", change.remote)},
				},
			)
		}
	}
...
func setupEgressIPWatcher(t *testing.T) (*egressIPWatcher, []string) {
	...
	flows, err := eip.oc.ovs.DumpFlows()
	if err != nil {
