...
func TestRecycleSync(t *testing.T) {
	...
	pendingPodObj, _ := api.Scheme.DeepCopy(runningPod)
	pendingPod := pendingPodObj.(*v1.Pod)
	...
	completedPodObj, _ := api.Scheme.DeepCopy(runningPod)
	completedPod := completedPodObj.(*v1.Pod)
