...
type ActualStateOfWorld interface {
	...
	// The uniqueName is only used when the volumeSpec is not available (crash
	// recovery).
	...
	AddVolumeNode(uniqueName api.UniqueVolumeName, volumeSpec *volume.Spec, nodeName types.NodeName, devicePath string) (api.UniqueVolumeName, error)
	...
	SetVolumeMountedByNode(volumeName api.UniqueVolumeName, nodeName types.NodeName, mounted bool, forceUnmount bool) error
...
func (asw *actualStateOfWorld) MarkVolumeAsAttached(
	uniqueName api.UniqueVolumeName, volumeSpec *volume.Spec, nodeName types.NodeName, devicePath string) error {
...
func (asw *actualStateOfWorld) AddVolumeNode(
	uniqueName api.UniqueVolumeName, volumeSpec *volume.Spec, nodeName types.NodeName, devicePath string) (api.UniqueVolumeName, error) {
	asw.Lock()
	defer asw.Unlock()
...
func (asw *actualStateOfWorld) SetVolumeMountedByNode(
	volumeName api.UniqueVolumeName, nodeName types.NodeName, mounted bool, forceUnmount bool) error {
	asw.Lock()
	defer asw.Unlock()
