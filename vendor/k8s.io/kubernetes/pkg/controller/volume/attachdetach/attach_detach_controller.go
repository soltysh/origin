...
func (adc *attachDetachController) populateActualStateOfWorld() error {
	...
	nodes := adc.nodeInformer.GetStore().List()
	for _, nodeObj := range nodes {
		node, ok := nodeObj.(*api.Node)
		if !ok {
			glog.Errorf("Error: expected node, got %+v", nodeObj)
			continue
		}
...
func (adc *attachDetachController) getNodeVolumeDevicePath(
	volumeName api.UniqueVolumeName, nodeName types.NodeName) (string, error) {
	var devicePath string
	var found bool
	nodeObj, exists, err := adc.nodeInformer.GetStore().GetByKey(string(nodeName))
...
func (adc *attachDetachController) populateDesiredStateOfWorld() error {
	...
	pods := adc.podInformer.GetStore().List()
	for _, podObj := range pods {
		podToAdd, ok := podObj.(*api.Pod)
		if !ok {
			glog.Errorf("Error: expected pod, got %+v", podObj)
			continue
		}
		adc.podAdd(&podToAdd)
		for _, podVolume := range podToAdd.Spec.Volumes {
			...
			volumeSpec, err := util.CreateVolumeSpec(podVolume, podToAdd.Namespace,
				adc.pvcInformer, adc.pvInformer)
			if err != nil {
				glog.Errorf(
...
func (adc *attachDetachController) processVolumesInUse(
	nodeName types.NodeName, volumesInUse []api.UniqueVolumeName, forceUnmount bool) {
