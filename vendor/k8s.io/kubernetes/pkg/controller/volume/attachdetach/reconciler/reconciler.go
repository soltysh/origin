			...
			if rc.isMultiAttachForbidden(volumeToAttach.VolumeSpec) {
				...
				if len(nodes) > 0 {
					if !volumeToAttach.MultiAttachErrorReported {
						errorMsgFmt := "(Volume : %q) from node %q failed to attach - volume is already exclusively attached to another node"
						erroMsg := fmt.Sprintf(errorMsgFmt, volumeToAttach.VolumeName, volumeToAttach.NodeName)
