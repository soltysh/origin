...
func RunHardPrune(oc *exutil.CLI, dryRun bool) (*RegistryStorageFiles, error) {
	pod, err := registryutil.GetRegistryPod(oc.AsAdmin().KubeREST())
	if err != nil {
		return nil, err
	}

	defer func(ns string) { oc.SetNamespace(ns) }(oc.Namespace())
	output, err := oc.AsAdmin().SetNamespace(kapi.NamespaceDefault).Run("env").Args("--list", "dc/docker-registry").Output()
	if err != nil {
		return nil, err
	}
	...
	err = wait.ExponentialBackoff(kclient.DefaultBackoff, func() (bool, error) {
		pruneType := "delete"
		if dryRun {
			pruneType = "check"
		}
		out, err := oc.SetNamespace(kapi.NamespaceDefault).AsAdmin().
...
func AssertDeletedStorageFiles(deleted, expected *RegistryStorageFiles) error {
	...
	return kerrutil.NewAggregate(errors)
}
...
func (c *CleanUpContainer) Run() {
	for image := range c.imageNames {
		err := c.OC.AsAdmin().REST().Images().Delete(image)
		if err != nil {
			fmt.Fprintf(g.GinkgoWriter, "clean up of image %q failed: %v\n", image, err)
		}
	}
	for isName := range c.isNames {
		err := c.OC.AsAdmin().REST().ImageStreams(c.OC.Namespace()).Delete(isName)
		if err != nil {
	...
	for isTag := range c.isTags {
		...
		err := c.OC.REST().ImageStreamTags(c.OC.Namespace()).Delete(parts[0], parts[1])
		if err != nil {
