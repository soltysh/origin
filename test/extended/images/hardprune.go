...
var _ = g.Describe("[Feature:ImagePrune] Image hard prune", func() {
	...
	testHardPrune := func(dryRun bool) {
		...
		for _, imgName := range []string{baseImg1, baseImg2, baseImg3, baseImg4, childImg1, childImg2, childImg3} {
			img, err := oc.AdminClient().Images().Get(imgName)
			o.Expect(err).NotTo(o.HaveOccurred())
			imgs[imgName] = img
		...
		*/
		...
		err = oc.AdminClient().ImageStreamTags(oc.Namespace()).Delete("a", "latest")
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		err = oc.AdminClient().Images().Delete(childImg1)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		err = oc.AdminClient().ImageStreamTags(oc.Namespace()).Delete("a-tagged", "latest")
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		err = oc.AdminClient().Images().Delete(baseImg1)
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions,
		...
		err = oc.AdminClient().Images().Delete(childImg2)
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions,
		...
		err = oc.AdminClient().ImageStreams(oc.Namespace()).Delete("b")
		o.Expect(err).NotTo(o.HaveOccurred())
		delete(expectedDeletions.ManifestLinks, oc.Namespace()+"/b")
		err = oc.AdminClient().Images().Delete(baseImg2)
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions,
		...
		err = oc.AdminClient().Images().Delete(childImg3)
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions,
