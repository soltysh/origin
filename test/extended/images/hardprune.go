...
var _ = g.Describe("[Feature:ImagePrune] Image hard prune", func() {
	...
	g.JustBeforeEach(func() {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		err = oc.AsAdmin().SetNamespace(kapi.NamespaceDefault).Run("adm").
			Args("policy", "add-cluster-role-to-user", "system:image-pruner",
				fmt.Sprintf("system:serviceaccount:%s:registry", kapi.NamespaceDefault)).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	testHardPrune := func(dryRun bool) {
		...
		for _, imgName := range []string{baseImg1, baseImg2, baseImg3, baseImg4, childImg1, childImg2, childImg3} {
			img, err := oc.AsAdmin().REST().Images().Get(imgName)
			o.Expect(err).NotTo(o.HaveOccurred())
			imgs[imgName] = img
		...
		*/
		...
		err = oc.AsAdmin().REST().ImageStreamTags(oc.Namespace()).Delete("a", "latest")
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		err = oc.AsAdmin().REST().Images().Delete(childImg1)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		err = oc.AsAdmin().REST().ImageStreamTags(oc.Namespace()).Delete("a-tagged", "latest")
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		err = oc.AsAdmin().REST().Images().Delete(baseImg1)
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions,
		...
		err = oc.AsAdmin().REST().Images().Delete(childImg2)
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions,
		...
		err = oc.AsAdmin().REST().ImageStreams(oc.Namespace()).Delete("b")
		o.Expect(err).NotTo(o.HaveOccurred())
		delete(expectedDeletions.ManifestLinks, oc.Namespace()+"/b")
		err = oc.AsAdmin().REST().Images().Delete(baseImg2)
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions,
		...
		// TODO: 3.3 cluster doesn't delete manifest configs
		...
		err = oc.AsAdmin().REST().Images().Delete(childImg3)
		o.Expect(err).NotTo(o.HaveOccurred())
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions,
